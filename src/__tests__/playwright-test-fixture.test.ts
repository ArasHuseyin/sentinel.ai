...
function makeMockSentinel() {
  const mock: any = {
    ...
    run: jest
      .fn<any>()
      .mockResolvedValue({ goalAchieved: true, success: true, totalSteps: 1, message: 'ok', history: [] }),
    screenshot: jest.fn<any>().mockResolvedValue(Buffer.from('PNG')),
    describeScreen: jest.fn<any>().mockResolvedValue('A page with a form'),
    getTokenUsage: jest
      .fn<any>()
      .mockReturnValue({ totalTokens: 50, estimatedCostUsd: 0.000005, totalInputTokens: 30, totalOutputTokens: 20 }),
    get page() {
...
describe('Playwright Test Fixture: runAiFixture', () => {
  it('calls sentinel.init() before use() and sentinel.close() after', async () => {
    ...
    mock.init.mockImplementation(async () => {
      callOrder.push('init');
    });
  ...
  it('throws descriptive error when apiKey is missing', async () => {
    ...
    await expect(runAiFixture({}, async () => {})).rejects.toThrow('apiKey is required');
  ...
  it('merges sentinelOptions over defaults (headless: false, verbose: 1)', async () => {
    ...
    await runAiFixture({ apiKey: 'test-key', headless: false, verbose: 1 } as any, async () => {}, SentinelClass);
