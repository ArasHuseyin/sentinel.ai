 ...
 */
function makeEnrichmentPage(contextMap: Record<number, string>, url = 'https://example.com', title = 'Example') {
  const self: any = {
    url: () => url,
    title: jest.fn(async () => title),
...
describe('StateParser', () => {
  ...
  it('caches state within TTL', async () => {
    ...
    const axCalls = (cdp.send as jest.Mock).mock.calls.filter((c: any[]) => c[0] === 'Accessibility.getFullAXTree');
  ...
  it('assigns incrementing ids to elements', async () => {
    const nodes = [makeNode('button', 'A', 1), makeNode('link', 'B', 2), makeNode('textbox', 'C', 3)];
    const cdp = makeMockCDP(nodes, [makeBoxModel(), makeBoxModel(), makeBoxModel()]);
    const page = makeMockPage();
    ...
    function makePageWithFrames(
      ...
      const frames: any[] = [
        ...
        ...childFrames.map((cf, idx) => {
          const frameEl = cf.iframeRect ? { boundingBox: jest.fn(async () => cf.iframeRect) } : null;
          return {
