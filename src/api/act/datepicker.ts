      ...
      .evaluate(
        ({ year, month, day }: { year: number; month: number; day: number }) => {
          ...
          const cells = Array.from(
            root.querySelectorAll<HTMLElement>('[role="gridcell"], [role="button"], button, td[role], [data-day]')
          ).filter(
            c =>
              c.offsetParent !== null &&
    ...
    const navigated = await page
      .evaluate(
        ({ dir }: { dir: number }) => {
          ...
          const btns = Array.from(scope.querySelectorAll<HTMLElement>('button, [role="button"]')).filter(
