 ...
 *
 * The accessibility tree does not describe compound widgets: a date picker, a
 * custom dropdown or a star rating built from divs has no role that says so.
 * This pass runs in the page and recognises them structurally — by ARIA
 * contract first, then by DOM shape — and pierces shadow roots so widgets
 * rendered inside Web Components are found too.
 *
 * Extracted from StateParser: it captures nothing from the class beyond the
 * page handle, and at ~500 lines it was by far the largest method there.
 */
export async function parseWidgetPatterns(
  page: Page,
  counter: { n: number }
): Promise<UIElement[]> {
  const rawWidgets = await page
    .evaluate(() => {
      ...
      for (const input of Array.from(comboboxInputs)) {
        ...
        const rect =
          triggerRect.width >= 10 && triggerRect.height >= 10
            ? triggerRect
      ...
      const popupTriggers = queryShadowAll(
      ...
      for (const trigger of Array.from(popupTriggers)) {
        ...
        const name =
          htmlEl.getAttribute('aria-label') || htmlEl.textContent?.trim().slice(0, 80) || '';
        if (!name) continue;
      ...
      for (const label of Array.from(labels)) {
        ...
        if (!parent) continue;
        const customWidget = parent.querySelector(
      ...
      for (const tablist of Array.from(tablists)) {
        ...
        const name =
          htmlEl.getAttribute('aria-label') || tabNames.join(' | ').slice(0, 80) || 'tabs';
        ...
        for (const widget of Array.from(libraryWidgets)) {
          ...
          const name =
            (() => {
              ...
              const ariaLabel =
                htmlEl.getAttribute('aria-label') || inputEl?.getAttribute('aria-label');
