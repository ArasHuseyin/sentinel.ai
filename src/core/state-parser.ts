...
export class StateParser {
  ...
  private async parseWidgetPatterns(counter: { n: number }): Promise<UIElement[]> {
    const rawWidgets = await this.page
      .evaluate(() => {
        const results: any[] = [];
        const seen = new Set<string>();
        ...
        function findLabelFor(el: Element): HTMLElement | null {
          ...
          return (
            (root.querySelector(`label[for="${CSS.escape(id)}"]`) as HTMLElement | null) ?? null
          );
        }
  ...
  private async enrichAndDetectRegions(elements: UIElement[]): Promise<void> {
    ...
    const results: { id: number; context: string; region: string; error: string }[] =
      await this.page
        .evaluate(
          ({
            items,
