...
export class SentinelDriver {
  ...
  private async resolveLauncher(browserType: BrowserType): Promise<typeof chromium> {
    const stock = browserType === 'firefox' ? firefox : browserType === 'webkit' ? webkit : chromium;
    ...
    try {
      ...
      const extraLauncher =
        browserType === 'firefox' ? extraMod.firefox : browserType === 'webkit' ? extraMod.webkit : extraMod.chromium;
